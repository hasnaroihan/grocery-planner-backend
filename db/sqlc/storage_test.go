...
func TestNewRecipeTx(t *testing.T) {
	...
	require.Equal(t, result.Recipe.Author, arg.Author)
	require.Equal(t, result.Recipe.Portion, arg.Portion)
	require.Equal(t, result.Recipe.Steps, arg.Steps)
	require.Len(t, result.Ingredients, len(arg.ListIngredients))

	for _,row := range result.Ingredients {
		idx := slices.IndexFunc(arg.ListIngredients, func(i ListIngredientParam) bool {return i.Name == row.Ingredient.Name})
		require.NotNil(t, idx)
		require.NotZero(t, row.Ingredient.ID)
		require.Equal(t, row.Amount, arg.ListIngredients[idx].Amount)
		require.Equal(t, row.UnitID, arg.ListIngredients[idx].UnitID)
	}
}
